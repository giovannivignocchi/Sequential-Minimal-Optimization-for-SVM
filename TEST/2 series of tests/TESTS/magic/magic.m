...
% Load the test set
xtest = readtable('C:\Users\giova\Dropbox\documenti\MATLAB\Sequential Minimal Optimization for SVM\Dataset\Data\in_magic_ts.txt');
ytest = readtable('C:\Users\giova\Dropbox\documenti\MATLAB\Sequential Minimal Optimization for SVM\Dataset\Data\out_magic_ts.txt');
xtest = xtest(:,1:end-1);
ytest = ytest(:,1:end-1);
...
violation = zeros(GridSize, modelsNumber);
...
for c=C
    for s = sigma
        ...
        if saveResult
            ...
            save(varFile,'trainingStats','kernelEval','iterNumber','svNumber','accuracy','modelParam');
        end
    end
end
